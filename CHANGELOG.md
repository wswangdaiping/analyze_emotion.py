...
本次改动围绕机器人动作编排能力扩展展开，在不改变原有分析流程、CLI 参数和 JSON 输出结构的前提下，补充新的动作描述，并扩大动作序列归一化时允许保留的动作编号范围。

------

### 修改 1：扩展 `ACTION_SYSTEM_PROMPT` 可用动作列表

在原有动作描述 `0-10` 之后新增动作 `11-15`：

| 编号 | 动作名称 | 主要用途 |
| ---- | -------- | -------- |
| 11 | 鞠躬谢谢 | 感谢、道歉、谢幕、正式致意 |
| 12 | 下蹲保护 | 害怕、受到威胁、委屈、退缩保护 |
| 13 | 挥手前进 | 开心迎接、主动靠近、欢迎到来 |
| 14 | 张手前进 | 欢迎、邀请、引导、展示成果 |
| 15 | 鞠躬下蹲挥手前进组合 | 故事高潮、表演压轴、复杂情绪转场 |

新增描述同时包含适用与不适用场景，便于模型在动作编排时区分单一情绪、复杂情绪和故事结尾高潮。
...
```python
ALLOWED_ACTIONS = set(range(0, 16))
```

使 `normalize_action_sequence()` 以及动作生成结果过滤逻辑能够保留 `11-15` 的新增动作编号。

------

### 修改 3：补充 `ACTION_MAP` 中文名称映射
...
这样当 `action_sequence` 中出现新增动作编号时，最终 JSON 的 `action_names` 字段可以返回对应中文名称，而不是 `"未知"`。

------

## 测试结果

OpenClaw 侧已完成测试，全部通过：

| 测试项 | 结果 |
| ------ | ---- |
| 脚本语法检查 | 通过 |
| `normalize_action_sequence()` 保留 `11-15` 合法动作编号 | 通过 |
| 非法动作编号过滤逻辑 | 通过 |
| DashScope 真实模型生成新增动作编号 | 通过 |
| `ACTION_MAP` 返回新增动作中文名称 | 通过 |
